...
```java
    //初始化BaseUrlManager ，默认加载 base_urls_config.json 配置
    //默认数据可以有多个环境，默认提供了两个key：DEBUG_CONFIG_KEY、RELEASE_CONFIG_KEY代表两个环境。你也可以参考在config文件中添加自定义 CUSTOM_CONFIG_KEY，可以通过setFileConfigKey来使用对应环境来实现打包时候默认数据切换。
