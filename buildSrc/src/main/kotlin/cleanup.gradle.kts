...
tasks.register("templateCleanup") {
    doLast {
        ...
        file("gradle.properties").replace(
            "com.logan.multiurlmanager",
            "com.github.$owner.$name"
        )

        file("buildSrc/src/main/kotlin/publish.gradle.kts").apply {
            replace("logan0817/MultiUrlManager", "$owner/$name")
            replace("logan0817/MultiUrlManager", "$owner/$name")
            replace("logan0817", owner)
            replace("logan", owner)
...
fun changePackageName(owner: String, name: String) {
    srcDirectories().forEach {
        it.walk().filter {
            ...
            it.replace("com.logan.multiurlmanager", "com.github.$owner.$name")
        }
    }
    srcDirectories().forEach {
        it.listFiles()!!.filter { it.isDirectory } // down to src/main
            ...
            .forEach {
                ...
                File(it, "com/logan/multiurlmanager").renameTo(newDir)
                File(it, "com").deleteRecursively()
            }
    }
}
