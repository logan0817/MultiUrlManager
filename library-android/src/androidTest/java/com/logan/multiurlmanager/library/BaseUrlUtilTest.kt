package com.logan.multiurlmanager.library

import android.widget.Toast
import androidx.test.ext.junit.runners.AndroidJUnit4
import androidx.test.platform.app.InstrumentationRegistry
import com.logan.multiurlmanager.library.bean.BaseUrl
import com.logan.multiurlmanager.library.utils.BaseUrlUtil
import org.junit.Assert.assertEquals
import org.junit.Test
import org.junit.runner.RunWith

/**
 * Instrumented test, which will execute on an Android device.
 *
 * See [testing documentation](http://d.android.com/tools/testing).
 */
@RunWith(AndroidJUnit4::class)
class BaseUrlUtilTest {

    @Test
    fun testLoadDynamicBaseUrlConfigs() {
        val context = InstrumentationRegistry.getInstrumentation().targetContext
        BaseUrlUtil.put(context, BaseUrl("testKey", "https://www.google.com/", true,"Test"))

        val value = BaseUrlUtil.loadDynamicBaseUrlConfigs(context)

        assertEquals("https://www.google.com/", value.values.firstOrNull()?.url)
    }
}
